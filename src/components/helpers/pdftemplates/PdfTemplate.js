...
const PdfTemplate = () => {
    ...
    const add = async (form) => {
        setLoading(true);
        let tmp_res = api.request('/api/pdf-template', 'POST', form, true)
            .then(res => {
                if (res.status===200||res.status===201){
    ...
    const edit = async (id, form) => {
        setLoading(true);
        let tmp_res = api.request('/api/pdf-template/'+id, 'POST', form, true)
            .then(res => {
                if (res.status===200||res.status===201){
                    tmp_res = {'status': 'ok', 'data': res.data.data};
                }else {
    ...
    const fdelete = async (id) => {
        setLoading(true);
        let tmp_res = api.request('/api/pdf-template/'+id, 'DELETE')
            .then(res => {
                if (res.status===200||res.status===201){
        ...
        <>
            <ContextData.Provider value={
                ...
                <ContextCrud.Provider value={
                    ...
                    <Collect MainContent={PdfTemplatePage} />
