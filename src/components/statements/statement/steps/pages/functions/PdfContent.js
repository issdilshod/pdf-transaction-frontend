...
class PdfContent {
    ...
    #get_daily_balance_content_with_real_data(dailyBalances){
        ...
        for (let key in dailyBalances){
            ...
            // amount pos
            let negative = 0;
            if (this.#determine_is_negative(dailyBalances[key]['ending_balance'])) { negative = this.offset.negative.x; }
            let digit = this.#determine_digit_of_amount(dailyBalances[key]['ending_balance']) - negative;

            oneContent = oneContent.replace('{amount_x_pos}', this.offset.daily_balances[digit]);
